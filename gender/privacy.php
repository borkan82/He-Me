<?php
$title = "Instant Name Gender Checker - Identify Gender by Name Quickly - Privacy";
...
$dbpdo           = new DbPDO();
$getLanguage     = $dbpdo->fetchOne("SELECT * FROM tab_languages WHERE `language_code` = '". $lang ."'");
if(!empty($getLanguage)){
    $langId = $getLanguage['id'];
}
$pageTranslation = $dbpdo->fetchOne("SELECT * FROM gender_translations WHERE `language_id` = ". $langId ." AND `page_id` = ". $pageId);
?>
<style>
    .home {
        height:auto;
